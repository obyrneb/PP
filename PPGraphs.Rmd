...
plot4 <- ggplot(services.df, aes(x = Sector, fill = Service)) +
...
plot2 <- ggplot(work.df, aes(x = Employee, y = Hours, fill = Task)) +
  ...
  facet_grid(Task~., 
             ...
             scales = "free_y",
             space = "free_y") +
  scale_y_continuous(breaks = c(20,40,60,80,100,200), expand_scale(add = c(0,10))) +
  theme(
...
PIA.df <- PIA.df %>%
  mutate(duration = 
           paste0(as.character(round(12 * as.numeric(as.yearmon(End) - as.yearmon(Start))),0)," months")) %>%
  filter(Type == "PIA")

status.clr <- c("Complete" = "#00BFC4", "Ongoing" = "#F8766D")
status.ln <- c("Complete" = 1, "Ongoing" = 2)

plot3 <- ggplot(PIA.df, aes(colour = Status)) +
  labs(title = "PIA Timelines", 
       subtitle = "The dotted line represents when data was collected (2018-10-10).",
       caption = "Data compiled by TBS's Internal Audit and Evalution Bureau") +
  ...
  geom_vline(xintercept = as.Date("2018-10-10"), linetype = 3, colour = "grey45") +
  geom_text(aes(x=Start, y=Name, label = duration), vjust =  1.5, hjust = 0, size = 3, show.legend = FALSE) +
